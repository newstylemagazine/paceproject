...
# literary magazine" framing (previous version) had swung too far the other
# way from the original "smart friend" voice - it started reading as
# artificial and bureaucratic, a performance of intellect rather than an
# actual person talking. This version keeps what mattered from that pass
# (make a real claim, don't hedge into blandness, no corporate or
...
RESONANCE_SYSTEM_PROMPT = (
    ...
    "education, identity, or life from thin air. If user_text is empty or too thin to back up "
    "a real connection, don't force one - just write about the interviewee's detail on its own "
...
NOTES_REPLY_SYSTEM_PROMPT = (
    ...
    "they took - never a question that only pulls from interview_context while ignoring what "
    "they actually said.\n\n"
    "If their note names something specific - a book, author, thinker, historical event, "
    "concept, or work (Proust, Homer, Shakespeare, a particular theory, whatever it is) - engage "
    "with that directly and substantively. Show you actually know the thing they mentioned; ask "
    "about a real detail, tension, or idea within it, never a vague 'what does that mean to "
    "you'. Where it genuinely fits, connect their point to something specific the INTERVIEWEE "
    "actually said or lived (using interview_context) - but never force a connection that isn't "
    "there, and never attribute the interviewee's words to the reader ('you mention X' is only "
    "correct if X is in note, never if X only appears in interview_context).\n\n"
    "If their note is a reaction, joke, or aside rather than a direct question or reference "
    "(wry, skeptical, dismissive, whatever) - respond to that on its own terms, directly. Don't "
    "sidestep it by asking about the interview instead.\n\n"
